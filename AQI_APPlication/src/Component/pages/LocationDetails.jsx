...
const LocalityDetail = ({ isDarkMode }) => {
  ...
  return (
   <div className={`flex flex-col gap-6 w-full max-w-full mt-10 px-6 ${isDarkMode ? "text-white" : "text-black"}`}>
  {/* Top Section: Map and AQI Circle */}
  <div className="flex flex-col lg:flex-row h-[800px] gap-6">
    
    {/* Map Section */}
    <div className="w-full relative lg:w-full p-5 border-2 shadow-xl rounded-2xl dark:bg-gray-800">
      <LiveMap />
    </div>

    {/* AQI Circle Section */}
    <div
      className={`w-full lg:absolute lg:h-[500px] lg:mt-48 lg:ml-[990px]  lg:z-[50] lg:w-[400px] flex flex-col items-center p-6 border-2 shadow-xl  rounded-2xl text-center ${isDarkMode ? "bg-[#374151]" : "bg-gray-100"}`}
    >
  ...
  <div className="flex flex-col items-center lg:flex-row justify-center gap-6">
    ...
    {/* Uncomment if needed <GasesTable isDarkMode={isDarkMode} /> */}
  </div>
</div>
